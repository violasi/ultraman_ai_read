...
export default function HeroHallPage() {
  ...
  return (
    <div className="space-y-4 py-2">
      <h2 className="text-xl font-black text-gray-800 text-center">{theme.labels.heroHallFull}</h2>
      ...
      <div className="grid grid-cols-3 gap-3 pb-4">
        {heroStats.map(hero => {
          ...
          return (
            <div
              key={hero.id}
              className={`bg-[var(--color-bg-warm)] rounded-2xl p-4 border border-[var(--color-border)] shadow-[0_2px_0_var(--color-shadow)] flex flex-col items-center gap-2 transition-all active:scale-95 cursor-pointer ${
                dimmed ? 'opacity-50' : ''
              }`}
            ...
            >
              <div className="relative">
                ...
                {cardCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-[var(--color-primary)] text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow">
                    {cardCount}
                  </span>
                )}
              </div>
