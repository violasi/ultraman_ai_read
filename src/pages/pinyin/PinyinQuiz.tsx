...
export default function PinyinQuiz() {
  ...
  const { updateStory, unlockCard, isUnlocked, recordModuleCompletion, getAllPinyinStories } = useApp()
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-gray-500 mb-6">你答对了 {correctCount}/{totalQ} 题</p>
          ...
          {stars < 2 && <p className="text-sm text-gray-400 mb-6">答对2题以上可以获得奥特曼卡片哦！</p>}
          <div className="space-y-3">
            <button onClick={() => navigate(`/pinyin/story/${story.id}`)}
            ...
            <button onClick={() => navigate('/pinyin')}
              className="w-full py-3 rounded-2xl font-bold bg-blue-700 text-white hover:bg-blue-700 active:scale-[0.97] transition-all">
              返回故事列表
            </button>
          </div>
        </div>
      ...
      <div className="flex justify-center gap-2">
        {questions.map((_, i) => (
          <div key={i} className={`w-3 h-3 rounded-full transition-all ${
            i < currentQ ? 'bg-green-400' : i === currentQ ? 'bg-blue-700 scale-125' : 'bg-gray-200'
          }`} />
        ))}
      </div>
      <p className="text-center text-sm text-gray-400">第 {currentQ + 1}/{totalQ} 题</p>
      <QuizCard
        key={currentQ}
